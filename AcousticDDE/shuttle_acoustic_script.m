...
% % No-damp solve
[tVector, xVector] = DDESolve(tFinal, false, false, ...
...
hold on;
subplot(1,2,2);
plot(tVector, xVector(2,:));
hold on;
title('Velocity [m/s] vs. time [s]')

% Primary rarefaction solve
[tVector, xVector] = DDESolve(tFinal, true, false, ...
    m, L, p0, T0, gamma, R, c, rho0, cushLength, accelLength, pR, A_R, A_L);
...
%         imageSign = -imageSign;
    ...
    dx(2) = dx(2) - globalDamp * rho0*c/m * A_L * damping * dt;
%     disp(damping)
